...
def mixed_dim_projections(gb, cond=None):
    ...
    local MortarGrid.primary_to_mortar and MortarGrid.primary_to_secondary operators, but
    concatinated to one large operator giving the mapping from all mortar grids to all
    secondary/primary grids. The lower dimension is considered the secondary while the higher
    dimension is considered the primary.
    ...
    Returns:
    primary2mortar: Projection from faces of primary to cells of mortar
    secondary2mortar: Projection from cells of secondary to cells of mortar
    mortar2primary: Projection from cells of mortar to faces of primary
    mortar2secondary: Projection from cells of mortar to cells of secondary
    """
    ...
    primary2mortar = sps.csc_matrix((num_mortar_cells, gb.num_faces()))
    secondary2mortar = sps.csc_matrix((num_mortar_cells, gb.num_cells()))
    mortar2primary = sps.csc_matrix((gb.num_faces(), num_mortar_cells))
    mortar2secondary = sps.csc_matrix((gb.num_cells(), num_mortar_cells))
    for e, d in gb.edges():
        ...
        mortar2primary += Pm_l2g * mg.mortar_to_primary_int() * Pl
        mortar2secondary += Ps_l2g * mg.mortar_to_secondary_int() * Pl
        primary2mortar += Pg * mg.primary_to_mortar_avg() * Pm_g2l
        secondary2mortar += Pg * mg.secondary_to_mortar_avg() * Ps_g2l

    return primary2mortar, secondary2mortar, mortar2primary, mortar2secondary
    ...
    for e, d_e in gb.edges():
        ...
        if side == "secondary":
            ...
            if transpose:
                ...
                matrix[ind_r, ind_c] = d_e[pp.DISCRETIZATION_MATRICES][keyword][name]
        if side != "secondary" and side != "primary":
